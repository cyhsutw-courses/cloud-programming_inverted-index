...
public class SimilarityCombiner
		extends
		Reducer<DocumentSimilarityPair, ScoreArrayWritable, DocumentSimilarityPair, ScoreArrayWritable> {
	@Override
	public void reduce(DocumentSimilarityPair key,
		...
		if (isOrQuery) {
			...
			context.write(
					new DocumentSimilarityPair(key.getDocName(), cosSim),
					new ScoreArrayWritable(list.toArray(new ScoreWritable[list
							.size()])));
		} else {
			// for "and query"
			for (int i = 0; i < queryTf.length; i += 1) {
